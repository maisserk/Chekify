...
expressApp.post('/api/admin/reset-password', async (req, res) => {
  ...
  if (!identifier || !newPassword || !adminToken) return res.status(400).json({ error: 'Faltan campos obligatorios' });
  if (!adminApp) return res.status(500).json({ error: 'Firebase Admin not initialized' });
  try {
    const auth = getAuth(adminApp);
    const db = getFirestore(adminApp, DATABASE_ID);
    let decodedToken: any;
    try { decodedToken = await auth.verifyIdToken(adminToken); }
    catch { return res.status(401).json({ error: 'Sesión administrativa no válida o expirada' }); }
    const adminEmail = decodedToken.email || '';
    const adminDoc = await db.collection('users').doc(decodedToken.uid).get();
    const adminData = adminDoc.exists ? adminDoc.data() || {} : {};
    const isAuthorized = adminEmail === 'maisserk@gmail.com' || hasRole(adminData, 'Administrador', 'Admin');
    if (!isAuthorized) return res.status(403).json({ error: 'No tienes permisos suficientes para realizar esta acción' });
    let targetUid = '';
    try {
      const email = identifier.includes('@') ? identifier : `${identifier}@chekify.local`;
      targetUid = (await auth.getUserByEmail(email)).uid;
    } catch { targetUid = identifier; }
...
expressApp.post('/api/push/subscribe', async (req, res) => {
  ...
  if (!hasRole(userData, 'Supervisor', 'Administrador', 'Admin')) return res.status(403).json({ error: 'Only supervisors and administrators can enable push alerts' });
  try {
    const docId = Buffer.from(subscription.endpoint).toString('base64').replace(/[/+=]/g, '_').slice(0, 100);
    await db.collection('push_subscriptions').doc(docId).set({ subscription, uid: decodedToken.uid, role, plantId: userData.plantId || null, updatedAt: Date.now() }, { merge: true });
...
expressApp.post('/api/push/unsubscribe', async (req, res) => {
  ...
  try {
    ...
    if (snapshot.exists && snapshot.data()?.uid !== identity.decodedToken.uid) return res.status(403).json({ error: 'You cannot remove another user subscription' });
    await ref.delete();
...
expressApp.post('/api/push/test', async (req, res) => {
  ...
  try {
    ...
    if (!stored.exists || stored.data()?.uid !== identity.decodedToken.uid) return res.status(403).json({ error: 'Subscription is not registered to this user' });
    const payload = JSON.stringify({ title: '🔔 ¡Notificaciones Activadas!', body: 'Estás suscrito para recibir alertas instantáneas en tu dispositivo al reportarse hallazgos críticos.', url: '/', tag: 'test-notification', requireInteraction: true });
...
expressApp.post('/api/push/send-alert', async (req, res) => {
  ...
  const identity = await authenticateRequest(req, res);
  if (!identity) return;
  if (!hasRole(identity.userData, 'Supervisor', 'Administrador', 'Admin')) return res.status(403).json({ error: 'Only supervisors and administrators can send push alerts' });
  ...
  try {
    ...
    const payload = JSON.stringify({ title: title || '⚠️ ¡ALERTA: Hallazgo Reportado!', body: body || `Se reportó un hallazgo en ${areaName || 'Área general'} (${equipmentName || 'Equipo'})`, url: url || '/', tag: tag || `finding-alert-${findingId || Date.now()}`, priority: priority || 'Alta', findingId: findingId || null, areaName, equipmentName, reportedBy, requireInteraction: true });
    let sentCount = 0;
    const errors: string[] = [];
    const sendPromises = subsSnapshot.docs.map(async (doc) => {
      ...
      try {
        ...
        if (err?.statusCode === 404 || err?.statusCode === 410) await doc.ref.delete().catch(() => undefined);
        else errors.push(err?.message || 'Unknown push error');
      }
    });
